...
class StreamEditingViewModel @Inject constructor(
    ...
    private val isStreamUpdateAvailableObservable = Observable
    ...
    private var streamUpdateDisposable: Disposable? = null
    ...
    override val isStreamUpdateAvailable: LiveData<Boolean> = MutableLiveData<Boolean>().apply {
        isStreamUpdateAvailableObservable
            .observeOn(mainThreadScheduler)
            .subscribeBy(
                onNext = ::setValue,
                onError = applicationErrorsLogger::logError
        ...
        launchLongTermOperation operation@{
            ...
            if (newStartDateTime != null) {
                if (endDateTime != null && newStartDateTime.isAfter(endDateTime)) {
                    sendSystemMessage(R.string.fragment_stream_editing_error_start_date_time_is_after_end_date_time)
                    return@operation
                }

                if (newStartDateTime.isBeforeNow) {
                    sendSystemMessage(R.string.fragment_stream_editing_error_start_date_time_is_before_now)
                    return@operation
                }
            }

            if (endDate?.isBefore(newStartDate) == true) {
                sendSystemMessage(R.string.fragment_stream_editing_error_start_date_is_after_end_date)
                return@operation
            }
        ...
        launchLongTermOperation operation@{
            ...
            if (newEndDateTime != null) {
                if (startDateTime != null && newEndDateTime.isBefore(startDateTime)) {
                    sendSystemMessage(R.string.fragment_stream_editing_error_end_date_time_is_before_start_date_time)
                    return@operation
                }

                if (newEndDateTime.isBeforeNow) {
                    sendSystemMessage(R.string.fragment_stream_editing_error_end_date_time_is_before_now)
                    return@operation
                }
            }

            if (startDate?.isAfter(newEndDate) == true) {
                sendSystemMessage(R.string.fragment_stream_editing_error_end_date_is_before_start_date)
                return@operation
            }
...
//
//        streamUpdateDisposable?.dispose()
//
...
//            val errorMessage =
//                applicationContext.getString(R.string.fragment_stream_editing_error_necessary_fields_are_not_filled_in)
//
...
//
//        streamUpdateDisposable = myStreamRepository
//            .updateMyStream(
//                title,
//                description,
//                startDateTime,
...
//                        .getString(R.string.fragment_stream_editing_error_failed_to_update_stream_information)
//
//                    systemMessageSubject.onNext(errorMessage)
//
...
//            .addTo(disposables)
    ...
    override fun prepareToNavigateToLiveBroadcastingDestination() {
