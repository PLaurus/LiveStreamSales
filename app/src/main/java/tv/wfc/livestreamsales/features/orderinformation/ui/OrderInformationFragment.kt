            ...
            viewModel.orderStatus.observe(viewLifecycleOwner){ orderStatus ->
                when(orderStatus){
                    Order.Status.JUST_MADE -> {
                        ...
                        val backgroundColor = ContextCompat.getColor(context, R.color.orderInformation_orderStatus_created)
                        setBackgroundColor(backgroundColor)
                    }
                    Order.Status.IN_PROGRESS -> {
                        visibility = View.VISIBLE
                        text = getString(R.string.fragment_order_information_order_status_in_progress)
                        val backgroundColor = ContextCompat.getColor(context, R.color.orderInformation_orderStatus_in_progress)
                        setBackgroundColor(backgroundColor)
                    }
                    Order.Status.DONE -> {
                        visibility = View.VISIBLE
