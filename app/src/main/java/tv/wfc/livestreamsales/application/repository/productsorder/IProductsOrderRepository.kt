...
interface IProductsOrderRepository {
    ...
    /**
     * Sends request to order [products] to remote server.
     */
    fun orderProducts(products: List<OrderedProduct>): Completable

    /**
     * Sends request to order products that are currently in cart to remote server.
     * If request finished successfully local cart will be cleared.
     */
    fun orderProductsFromCart(): Completable
