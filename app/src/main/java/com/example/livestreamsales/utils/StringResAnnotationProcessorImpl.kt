...
class StringResAnnotationProcessorImpl : StringResAnnotationProcessor {
    ...
    override fun process(context: Context, @StringRes resId: Int){
        with(context){
            val fullText = getText(resId) as SpannedString
            val spannableString = SpannableString(fullText)
            val annotations = fullText.getSpans<Annotation>(0, fullText.length)
